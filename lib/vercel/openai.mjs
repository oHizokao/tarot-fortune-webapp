...
import { DEFAULT_TAROT_PROMPT, TAROT_ANSWER_SCHEMA, TAROT_RESPONSE_FORMAT, TAROT_SAFETY_GUARDRAILS } from "./settings.mjs";
import { hashText } from "./security.mjs";

const OPENAI_URL = "https://api.openai.com/v1/responses";
const TRANSIENT_STATUSES = new Set([429, 500, 502, 503, 504]);

export function buildOpenAiBody({ model, instructions, input, userId, maxOutputTokens = 900, responseFormat = "text" }) {
  const body = {
    model: String(model),
  ...
  if (responseFormat === "tarot") body.text = { format: { type: "json_schema", name: "tarot_reading", strict: true, schema: TAROT_ANSWER_SCHEMA } };
  return body;
}

export function composeTarotInstructions(prompt) {
  return [String(prompt || "").trim() || DEFAULT_TAROT_PROMPT, TAROT_SAFETY_GUARDRAILS, TAROT_RESPONSE_FORMAT].join("\n\n");
}

export function parseTarotAnswer(value) {
  let parsed;
  try { parsed = JSON.parse(String(value || "")); } catch { throw new AppError("AI ส่งรูปแบบคำตอบไม่ถูกต้อง กรุณาลองใหม่", 502, "EMPTY_AI_RESPONSE"); }
  const verdict = boundedText(parsed?.verdict, 1_500);
  const overallPrediction = boundedText(parsed?.overall_prediction, 4_000);
  const cards = Array.isArray(parsed?.cards)
    ? parsed.cards.map((card, index) => ({
      position: Number.isInteger(Number(card?.position)) ? Number(card.position) : index + 1,
      name: boundedText(card?.name, 240),
      meaning: boundedText(card?.meaning, 2_000),
      prediction: boundedText(card?.prediction, 3_000),
    })).filter((card) => card.name && card.meaning && card.prediction)
    : [];
  const safetyNote = boundedText(parsed?.safety_note, 1_500);
  if (!verdict || !overallPrediction || !cards.length) throw new AppError("AI ส่งคำตอบไม่ครบรูปแบบ กรุณาลองใหม่", 502, "EMPTY_AI_RESPONSE");
  return { verdict, cards, overall_prediction: overallPrediction, safety_note: safetyNote };
}

export function formatTarotAnswer(reading) {
  const lines = [`คำฟันธง: ${reading.verdict}`];
  reading.cards.forEach((card) => {
    lines.push(`ไพ่ใบที่ ${card.position} — ${card.name}`, `ความหมายของไพ่: ${card.meaning}`, `คำทำนาย: ${card.prediction}`);
  });
  lines.push(`คำทำนายโดยรวม: ${reading.overall_prediction}`);
  if (reading.safety_note) lines.push(`หมายเหตุ: ${reading.safety_note}`);
  return lines.join("\n\n");
}
...
export function buildTarotInput(question, metadata, conversation = []) {
  ...
  let input = `คำถามของผู้ใช้:\n${String(question).trim()}\n\nคำถามปัจจุบันต้องตอบตรงประเด็น (ห้ามเปลี่ยนไปตอบคำถามเก่า):\n${String(question).trim()}\n\nไพ่ที่เปิดจริง:\n${cardLines.join("\n")}\n\nกติกาสำคัญ: ให้คำถามปัจจุบันเป็นแกนหลักของคำตอบ ตอบให้ตรงเรื่องและระบุคำตอบในส่วนสรุปอย่างชัดเจน หากไพ่ไม่สามารถตอบข้อเท็จจริงนั้นได้ ให้บอกข้อจำกัดสั้น ๆ แล้วเชื่อมคำบนไพ่กับเจตนาของคำถามอย่างเป็นประโยชน์ ห้ามพูดกว้าง ๆ จนไม่ตอบคำถาม`;
  if (previous.length) input += `\n\nบริบทการสนทนาก่อนหน้า (ใช้เพื่อเชื่อมโยงเท่านั้น ห้ามกลบคำถามปัจจุบัน):\n${previous.join("\n")}`;
  return input;
}

export async function requestOpenAi({ settings, input, userId, maxOutputTokens = 900, responseFormat = "text", fetchImpl = globalThis.fetch, timeoutMs = 25_000 }) {
  ...
  const body = buildOpenAiBody({ model: settings.model, instructions: composeTarotInstructions(settings.prompt), input, userId, maxOutputTokens, responseFormat });
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (response.ok) {
      ...
      const structured = responseFormat === "tarot" ? parseTarotAnswer(answer) : null;
      return { answer: structured ? formatTarotAnswer(structured) : answer.slice(0, 12_000), structured, responseId: String(decoded.id || ""), usage: normalizeUsage(decoded.usage) };
    }
...
export async function createTarotResponse({ userId, question, cards, messages = [], settings, dailyLimit, queryFn = defaultQuery, fetchImpl = globalThis.fetch }) {
  ...
  try {
    return await requestOpenAi({ settings, input, userId, responseFormat: "tarot", fetchImpl });
  } catch (error) {
