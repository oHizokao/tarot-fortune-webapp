...
import { allocateDeckSlice, assertReadingOwner, cardMetadata, createDeckOrder, createReadingId, normalizeDeckSession, normalizeReading, normalizeRound, readingContext, validateReadingCards } from "../readings.mjs";
...
export async function listDeckSessions(request) {
  ...
  const current = await requireBetaUser(request);
  const rows = await query("SELECT * FROM reading_sessions WHERE user_id = $1 ORDER BY CASE WHEN status = 'active' THEN 0 ELSE 1 END, updated_at DESC LIMIT 20", [Number(current.user.id)]);
  return success({ sessions: rows.map((row) => normalizeDeckSession(row)) });
}
...
export async function drawReadingRound(request, id) {
  ...
  try {
    [roundRows] = await transaction([{
      ...
      INSERT INTO reading_rounds (id, session_id, round_number, question, cards, status, request_id, created_at, updated_at)
      SELECT $5, p.session_id, p.round_number, $6, p.selected_cards, 'drawn', $4, NOW(), NOW()
      FROM picked p
      JOIN advanced a ON a.id = p.session_id
      RETURNING *`,
      params: [sessionId, Number(current.user.id), count, requestId, roundId, question],
    }]);
  } catch (error) {
