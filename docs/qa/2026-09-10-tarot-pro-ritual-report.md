...
- Production serves `ai.css?v=20260910-ai-two-scene-v41`, `ai.js?v=20260910-ai-two-scene-v41`, and `data-ui-version="pro-ritual-v1"`.
- `GET /api/health` — HTTP 200; `ready`, `database`, `schema`, `admin`, and `ai` all reported true. Secret values were not recorded.
- Production guest smoke — 4 passed: three-card draw/reset, continuous non-repeating rounds, full card ratio/no overflow, and always-on ritual motion.
